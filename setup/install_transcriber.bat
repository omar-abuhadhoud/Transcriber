...
set "REPO_URL=https://github.com/Omemo7/Transcriber.git"
set "INSTALL_DIR=%USERPROFILE%\Transcriber"
set "BOOTSTRAP_LOG_DIR=%LOCALAPPDATA%\Transcriber\logs"
...
set "BOOTSTRAP_LOG=%BOOTSTRAP_LOG_DIR%\install_transcriber.log"

echo Transcriber installer
echo Log file: %BOOTSTRAP_LOG%
echo.
call :log ============================================================
call :log Transcriber installer started at %DATE% %TIME%
...
if errorlevel 1 (
    ...
    echo Install Python 3.10 and Git manually, then run setup\setup_windows_gpu.bat from the project folder.
...
if exist "%INSTALL_DIR%\.git" (
    call :progress 50 "Updating Transcriber source"
    echo Updating existing project...
    call :log Updating existing project with git pull.
    git -C "%INSTALL_DIR%" pull >> "%BOOTSTRAP_LOG%" 2>&1
) else (
    call :progress 50 "Downloading Transcriber source"
    echo Downloading Transcriber...
...
if errorlevel 1 (
    ...
    echo Check REPO_URL inside setup\install_transcriber.bat.
...
cd /d "%INSTALL_DIR%"
if not exist "%INSTALL_DIR%\setup\setup_windows_gpu.bat" (
    echo setup\setup_windows_gpu.bat was not found in the downloaded project.
    echo Make sure the latest setup files were committed and pushed to GitHub.
    call :log ERROR: setup\setup_windows_gpu.bat was not found after clone/update.
...
call :log Running setup\setup_windows_gpu.bat.
call "%INSTALL_DIR%\setup\setup_windows_gpu.bat" %*
if errorlevel 1 (
    echo Setup failed. See logs:
    echo %BOOTSTRAP_LOG%
    echo %INSTALL_DIR%\.transcriber_state\logs\setup_windows_gpu.log
...
echo %BOOTSTRAP_LOG%
if exist "%INSTALL_DIR%\.transcriber_state\logs\setup_windows_gpu.log" (
    echo %INSTALL_DIR%\.transcriber_state\logs\setup_windows_gpu.log
)
exit /b 0
