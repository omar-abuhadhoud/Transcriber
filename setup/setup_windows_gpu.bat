...
set "SETUP_LOG_DIR=%CD%\.transcriber_state\logs"
...
if exist ".venv\Scripts\python.exe" (
    .venv\Scripts\python.exe setup\setup_windows_gpu.py %*
) else (
    where py >nul 2>nul
    if not errorlevel 1 (
        py -3 setup\setup_windows_gpu.py %*
    ) else (
        python setup\setup_windows_gpu.py %*
    )
)
